...
from schemas.users import UserRequest, UpdateUserRequest
...
async def create_token(db: AsyncSession, user_id: int):
    # 生成 Token + 设置过期时间 -> 查询数据库当前是否有 Token ,有：更新，没有：添加
...
# 更新用户信息 update 更新 -> 检查是否命中 -> 获取更新后的用户返回
async def update_user_info(db: AsyncSession, username: str, user_data: UpdateUserRequest):
    ...
    # user_data 是一个 Pydantic 模型类，——> 模型类转换成字典 --> ** 解包
    # 没有设置值的不更新
...
async def change_password(db: AsyncSession, user: User, old_password: str, new_password: str):
    ...
    db.add(user)  # 最好加上这个，由SQLAlchemy 真正接管这个 User 对象，确保可以 commit。规避 session 过期或关闭导致的不能提交的问题
    await db.commit()
