...
class Category(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True,
                                    comment="分类ID")  # autoincrement= True 自增
...
class News(Base):
    ...
    description: Mapped[Optional[str]] = mapped_column(String(255), comment="新闻描述")
    ...
    def __repr__(self):  # 打印规则，如果不用这个方法，打印出来是对象所在的内存地址
        return f"<Category(id={self.id}, name={self.title}, sort_order={self.views})>"
